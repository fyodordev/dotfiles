...
WINDOW_ICONS = {
    'default-icon': fa.icons['list'],
    'telegram-desktop': u'\uF1D8',
    'web.whatsapp.com': fa.icons['whatsapp'],
    ...
    'mpv': fa.icons['film'],
    'vlc': fa.icons['film'],
    'filelight': fa.icons['chart-pie'],
    'code-oss': fa.icons['code'],
    'toggl.com__app_timer': fa.icons['stopwatch'],
    'calendar.google.com__calendar_b_0_r': fa.icons['calendar'],
    'www.onenote.com__notebooks': fa.icons['tasks'],
    ...
    'sxiv': fa.icons['image'],
    'empty-icon': fa.icons['ellipsis-h'],
}

# This icon is used for any application not in the list above
DEFAULT_ICON = fa.icons['list']
EMPTY_ICON = fa.icons['ellipsis-h'] 
CLIPBOARD_ICON = fa.icons['clipboard']
...
def rename_workspaces(i3, icon_list_format='default'):
    ...
    for ws_index, workspace in enumerate(i3.get_tree().workspaces()):
        ...
        #print(f"===WORKSPACE {str(workspace.name)} ===")
        ...
        # Inserted by fyodor        
        # Sort icons according to icon list, then pick the first one that matches.
        all_icons_list = list(WINDOW_ICONS.values())
        new_icons = sorted(
                icon_list,
                key=lambda icon: all_icons_list.index(icon)
        )
        #print(f"indexes {','.join([icon + str(all_icons_list.index(icon)) for icon in new_icons])}")
        #print('icons on ' + str(new_num) + ' are: ' + ';'.join(new_icons))
        if name_parts.num == "300":
            new_icons = CLIPBOARD_ICON
        elif new_icons:
            new_icons = new_icons[0]
        else:
            new_icons = EMPTY_ICON

        new_name = construct_workspace_name(
            NameParts(
                num=new_num, shortname=name_parts.shortname, icons=new_icons))
        #print(f"Name is {str(new_name)}")

        if workspace.name == new_name:
            continue
        #print(f'RENUMBER_WORKSPACES: {RENUMBER_WORKSPACES}')
        #print(f'new_num: {new_num} n: {n} name_parts.num: {name_parts.num}')
        #print('command: rename workspace "%s" to "%s"' % (workspace.name, new_name))
        i3.command(
